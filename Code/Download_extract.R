...
# Extract names of files from the site in that year
years <- 2016:1976
download.urls <-
    sapply(years, 
           function(year) {
...
# Download each file
for (year in years) {
    download.location <- paste0("DataFiles/Raw/", year)
    if (!dir.exists(download.location)) dir.create(download.location)
    ...
    for (url in urls.year) {
        ...
        try(unzip (temp, exdir = paste0("DataFiles/Raw/", year)))
        unlink(temp)
    }
}
...
write(urls.missed, file = "DataFiles/Raw/Unable_to_download.txt")
