...
cleaning <- function(input.dir = "Processed2", output.dir = "Cleaned2",
                     patent.colnames = c("Patent", "Date", "Order"), patent.coltypes = "cccd",
                     citation.colnames = c("Patent", "Citation", "Date"), citation.coltypes = "ccdcc") {
    ...
    #     # Add Column Names
    #     df <- read_csv(file, col_names = T, col_types = "cdccd")
    ...
    for (file in citation.files[15:15]) {
        gc()
        print(file)
        # Add Column Names
        df <- read_csv(file, col_names = T, col_types = citation.coltypes)
        ...
        # }
        # Remove the 0s from text patent numbers
        type_txt <- str_extract(df$Patent, "^(D|RE|PP|H|T)")
        type_txt[is.na(type_txt)] <- ""
        rem0_txt <- sub("^(D|RE|PP|H|T)*(0|\\.)+", "", df$Patent)
        rem0_txt <- str_trim(rem0_txt)
        df$Patent <- paste0(type_txt, rem0_txt)
